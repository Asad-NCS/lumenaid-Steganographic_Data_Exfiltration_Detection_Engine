    ...
    def run(self, file_path: str, user_id: int) -> ScanResult:
        ...
        try:
            ...
            if file_type == 'PNG':
                ...
                if _iend_pos != -1:
                    _expected_end = _iend_pos + len(_iend_marker)
                    _appended_bytes = len(_png_bytes) - _expected_end
                    if _appended_bytes > 64:
                        with pg_conn.cursor() as cur:
                            cur.execute(
                                "INSERT INTO alerts (file_id, severity, description) VALUES (%s, 'CRITICAL', %s)",
                                (file_id, f"Signal 5 — PNG Structure Anomaly: {_appended_bytes} bytes detected after IEND marker. Consistent with appended steganographic payload.")
                            )
                            cur.execute(
                                """
                                UPDATE files
